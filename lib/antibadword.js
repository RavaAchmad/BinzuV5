...
import { isSudo, normalizeJid } from './index.js';
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (!bot || (bot.admin !== 'admin' && bot.admin !== 'superadmin' && bot.admin !== true)) {
        return;
    }

    const participant = groupMetadata.participants.find((p) => normalizeJid(p.id) === senderNormalized);
    if (participant && (participant.admin === 'admin' || participant.admin === 'superadmin' || participant.admin === true)) {
        return;
    }
    try {
