                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            // Recovery attempt for malformed JSON (e.g. single quotes)
                            console.warn(`[STORE] Malformed JSON detected for ${chatId}:${key}, attempting recovery...`);
                            // Replace single quotes used for strings with double quotes if possible, or attempt normalization
                            const fixedValue = rawValue.replace(/'/g, '"');
                            const parsed = JSON.parse(fixedValue);
                            // Persist the fixed version
                            await this.saveSetting(chatId, key, parsed);
                            return parsed;
                        }
                    }
                    finally {
            ...
            getSetting(chatId, key) {
                try {
                    ...
                    try {
                        ...
                        console.warn(`[SQLITE] Malformed JSON detected for ${chatId}:${key}, attempting recovery...`);
                        try {
                            // Normalize single quotes to double quotes for recovery
                            const fixedValue = rawValue.replace(/'/g, '"');
                            const parsed = JSON.parse(fixedValue);
                            // Resave the properly serialized JSON
                            saveSettingStmt.run(chatId, key, JSON.stringify(parsed), Date.now());
                            return parsed;
                        } catch (err2) {
                            console.error(`[SQLITE] Failed to recover malformed JSON for ${chatId}:${key}:`, err2.message);
                            return null;
                        }
                    }
                }
