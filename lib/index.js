...
function normalizeJid(jid) {
    ...
    const [user] = jidStr.split('@')[0].split(':');
    return `${user}@s.whatsapp.net`.toLowerCase();
}

async function isSudo(userId) {
    try {
        const data = await loadUserGroupData();
        if (!data.sudo) return false;
        const normalized = normalizeJid(userId);
        return data.sudo.some(s => normalizeJid(s) === normalized);
    }
    catch (error) {
...
async function addSudo(userJid) {
    try {
        ...
        if (!data.sudo.some(s => normalizeJid(s) === normalized)) {
...
async function removeSudo(userJid) {
    try {
        ...
        if (!data.sudo)
            data.sudo = [];
        const normalized = normalizeJid(userJid);
        const filtered = data.sudo.filter(s => normalizeJid(s) !== normalized);
        if (filtered.length !== data.sudo.length) {
            data.sudo = filtered;
            await saveUserGroupData(data);
        }
        return true;
